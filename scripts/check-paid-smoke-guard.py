...
def main() -> None:
    ...
    # The helper itself must never run automatically.
    ...
    # Authorization and identity must fail closed on the exact current main SHA.
    required_fragments = {
    ...
    # The dispatcher must not read provider credentials itself.
    if "secrets." in text or "${{ secrets" in text:
        fail("paid smoke dispatcher must not read provider secrets")

    # Remote actions must use immutable commit references.
    ...
    # Global token is read-only; actions:write is isolated to the dispatch job.

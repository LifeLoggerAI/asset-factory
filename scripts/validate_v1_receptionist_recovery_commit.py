...
from validate_v1_receptionist_recovery_marker import MARKER_PATH, validate_marker
...
def validate_commit(commit_sha: str, marker_path: Path = MARKER_PATH) -> str:
    ...
    changes = git_output('diff', '--name-status', '--no-renames', parent_sha, commit_sha).splitlines()
    expected = [f'A\t{marker_path.as_posix()}']
    if changes != expected:
        raise ValueError(f'authorization commit must add exactly the recovery marker; actual={changes!r}, expected={expected!r}')
...
def main() -> int:
    ...
    parent_sha = validate_commit(args.commit, args.marker)

...
def main():
 ...
 for meta in sorted(GEN.rglob('*.render.json')):
  try: data=json.loads(meta.read_text()); data['sourceReceipt']=str(meta.relative_to(ROOT)); receipts.open('a').write(json.dumps(data,sort_keys=True)+'\n')
 ...
 (OUT/'README.md').write_text('# Creative review package\n\nRun `python scripts/build_creative_review.py`. Scores are technical heuristics, not human aesthetic approval. Contact sheets require the optional renderer added by CI when images exist.\n')

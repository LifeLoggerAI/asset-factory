...
`forge_versioned_legacy.py`, `version_catalog.json`, and the pre-canonical manifest builders are compatibility/source-material files. They do not define public release numbering and must not be executed directly or used as release evidence.
...
The check builds every canonical manifest without making a provider call and fails on label/count/path/manifest drift, duplicate names, or duplicate canonical paths.

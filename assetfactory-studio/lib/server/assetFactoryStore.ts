...
export function getStoreDiagnostics() {
  ...
  return { mode: getStoreMode(), fallbackActive: !d.available, firebase: d };
}

export async function addJob(job: GenericRecord) { return localAddJob(job); }
export async function readJobs() { return localReadJobs(); }
export async function findJob(jobId: string) { return localFindJob(jobId); }
export async function updateJob(jobId: string, patch: GenericRecord) { return localUpdateJob(jobId, patch); }
...
export async function materializeAsset(jobId: string) {
  ...
  if (!job) return null;
  const rendered = await renderAsset(job as GenericRecord & { jobId: string; prompt: string; type: string; tenantId?: string });
  await localWriteGenerated(rendered.assetFileName, rendered.assetBuffer);
  await localWriteGenerated(`${jobId}.json`, Buffer.from(JSON.stringify(rendered.manifest, null, 2)));
  const asset = {
    jobId,
...
export async function approveAsset(jobId: string, approvalPatch: GenericRecord) { return { jobId, ...approvalPatch, approvalId: randomUUID() }; }
export async function createAssetVersion(jobId: string, versionPatch: GenericRecord) { return { jobId, versionId: randomUUID(), ...versionPatch }; }

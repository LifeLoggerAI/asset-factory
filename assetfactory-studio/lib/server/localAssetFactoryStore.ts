...
type Db = { jobs: Record<string, GenericRecord>; assets: Record<string, GenericRecord> };
...
export async function localUpdateJob(jobId: string, patch: GenericRecord) {
  ...
  db.jobs[jobId] = { ...db.jobs[jobId], ...patch, updatedAt: new Date().toISOString() };
  await writeDb(db);
...
export async function localWriteGenerated(fileName: string, buffer: Buffer) {
  ...
  const p = path.join(generatedDir, fileName);
  await fs.writeFile(p, buffer);
  return p;
}
export async function localReadGenerated(fileName: string) {
  try { return await fs.readFile(path.join(generatedDir, fileName)); } catch { return null; }
}

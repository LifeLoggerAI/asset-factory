...
async function postJson(url: string, headers: Record<string, string>, body: JsonRecord) {
  ...
  if (!response.ok) throw new Error(`Provider request failed ${response.status}: ${typeof payload === 'string' ? payload : JSON.stringify(payload)}`);
...
async function getJson(url: string, headers: Record<string, string>) {
  ...
  const payload = await parseResponsePayload(response);
  if (!response.ok) throw new Error(`Provider poll failed ${response.status}: ${typeof payload === 'string' ? payload : JSON.stringify(payload)}`);
  return payload as JsonRecord;
}
...
async function renderOpenAi(input: GenerateRequest, definition: AssetTypeDefinition): Promise<ProviderRenderResult | null> {
  ...
  if (definition.canonicalType === 'graphic') {
    const size = input.size?.width && input.size?.height ? `${input.size.width}x${input.size.height}` : env('ASSET_FACTORY_GRAPHICS_SIZE') || '1024x1024';
    const model = env('ASSET_FACTORY_GRAPHICS_MODEL') || 'gpt-image-1';
    const payload = await postJson('https://api.openai.com/v1/images/generations', { authorization: `Bearer ${apiKey}` }, { model, prompt: input.prompt, size, response_format: 'b64_json' });
    ...
    if (b64) return { assetBuffer: Buffer.from(b64, 'base64'), assetMimeType: 'image/png', extension: 'png', metadata: { provider: 'openai', providerModel: model, providerOutput: 'b64_json' } };
    if (url) {
      const binary = await fetchBinary(url);
      return { assetBuffer: binary.buffer, assetMimeType: binary.mimeType, extension: extensionFromMime(binary.mimeType, 'png'), metadata: { provider: 'openai', providerModel: model, providerOutput: 'url' } };
    }
  ...
  if (definition.canonicalType === 'audio') {
    ...
    const response = await fetch('https://api.openai.com/v1/audio/speech', { method: 'POST', headers: { authorization: `Bearer ${apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ model, voice, input: input.prompt, response_format: 'wav' }) });
    if (!response.ok) throw new Error(`OpenAI audio request failed ${response.status}: ${await response.text()}`);
    return { assetBuffer: Buffer.from(await response.arrayBuffer()), assetMimeType: response.headers.get('content-type') ?? 'audio/wav', extension: 'wav', metadata: { provider: 'openai', providerModel: model, voice } };
  }
...
async function renderElevenLabs(input: GenerateRequest): Promise<ProviderRenderResult | null> {
  ...
  const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, { method: 'POST', headers: { 'xi-api-key': apiKey, 'content-type': 'application/json', accept: 'audio/mpeg' }, body: JSON.stringify({ text: input.prompt, model_id: modelId }) });
  if (!response.ok) throw new Error(`ElevenLabs audio request failed ${response.status}: ${await response.text()}`);
  return { assetBuffer: Buffer.from(await response.arrayBuffer()), assetMimeType: response.headers.get('content-type') ?? 'audio/mpeg', extension: 'mp3', metadata: { provider: 'elevenlabs', providerModel: modelId, voiceId } };
}
...
async function renderStability(input: GenerateRequest): Promise<ProviderRenderResult | null> {
  ...
  const response = await fetch(`https://api.stability.ai/v2beta/stable-image/generate/${engine}`, { method: 'POST', headers: { authorization: `Bearer ${apiKey}`, accept: 'image/*' }, body: form });
  if (!response.ok) throw new Error(`Stability image request failed ${response.status}: ${await response.text()}`);
  const mimeType = response.headers.get('content-type') ?? 'image/png';
  return { assetBuffer: Buffer.from(await response.arrayBuffer()), assetMimeType: mimeType, extension: extensionFromMime(mimeType, 'png'), metadata: { provider: 'stability', providerModel: engine } };
}
...
async function renderReplicate(input: GenerateRequest, definition: AssetTypeDefinition): Promise<ProviderRenderResult | null> {
  ...
  const version = definition.canonicalType === 'model3d' ? env('ASSET_FACTORY_MODEL3D_MODEL') : definition.canonicalType === 'audio' ? env('ASSET_FACTORY_AUDIO_MODEL') : env('ASSET_FACTORY_GRAPHICS_MODEL');
  if (!apiKey || !version) return null;
  const prediction = await postJson('https://api.replicate.com/v1/predictions', { authorization: `Token ${apiKey}` }, { version, input: { prompt: input.prompt } });
  ...
  return { assetBuffer: binary.buffer, assetMimeType: binary.mimeType, extension: extensionFromMime(binary.mimeType, definition.extension), metadata: { provider: 'replicate', providerModel: version, predictionId: current.id } };
}
...
async function renderFal(input: GenerateRequest, definition: AssetTypeDefinition): Promise<ProviderRenderResult | null> {
  ...
  const model = definition.canonicalType === 'model3d' ? env('ASSET_FACTORY_MODEL3D_MODEL') : definition.canonicalType === 'audio' ? env('ASSET_FACTORY_AUDIO_MODEL') : env('ASSET_FACTORY_GRAPHICS_MODEL');
  if (!apiKey || !model) return null;
  const payload = await postJson(`https://fal.run/${model}`, { authorization: `Key ${apiKey}` }, { prompt: input.prompt });
  const outputUrl = firstUrl(payload);
  if (!outputUrl) throw new Error('Fal response did not return a downloadable output URL');
  const binary = await fetchBinary(outputUrl);
  return { assetBuffer: binary.buffer, assetMimeType: binary.mimeType, extension: extensionFromMime(binary.mimeType, definition.extension), metadata: { provider: 'fal', providerModel: model } };
}

export async function renderWithConfiguredProvider(input: GenerateRequest, definition: AssetTypeDefinition): Promise<ProviderRenderResult | null> {
  ...
  if (!result) throw new Error(`Configured provider ${provider} cannot render ${definition.canonicalType} or is missing required env`);
  return result;
}

async function renderProvider(provider: AssetProviderName, input: GenerateRequest, definition: AssetTypeDefinition): Promise<ProviderRenderResult | null> {

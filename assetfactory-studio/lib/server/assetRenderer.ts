...
import type { RendererMode } from './assetFactoryTypes';

type AssetSize = { width?: number; height?: number };
...
function renderGraphic(input: GenerateRequest & Record<string, unknown>, width: number, height: number) {
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><linearGradient id="g" x1="0" x2="1" y1="0" y2="1"><stop offset="0%" stop-color="${background}"/><stop offset="100%" stop-color="#101828"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/><circle cx="${Math.round(width * 0.78)}" cy="${Math.round(height * 0.24)}" r="${Math.round(Math.min(width, height) * 0.16)}" fill="${accent}" opacity="0.65"/><rect x="${Math.round(width * 0.08)}" y="${Math.round(height * 0.62)}" width="${Math.round(width * 0.76)}" height="${Math.round(height * 0.16)}" rx="28" fill="${accent2}" opacity="0.5"/><text x="40" y="90" fill="#e0f2fe" font-size="42">${escapeSvgText(input.jobId)}</text><text x="40" y="150" fill="#f8fafc" font-size="28">${escapeSvgText(text)}</text></svg>`);
}

function renderModel(input: GenerateRequest & Record<string, unknown>) {
  const hash = stableHash(input);
  const color = colorFromHash(hash, 1);
  const spatialModelContract = normalizeSpatialModelContract((input.metadata as Record<string, unknown> | undefined)?.spatialModelContract);
  return Buffer.from(JSON.stringify({
    ...
    materials: [{ name: 'hash-derived-material', pbrMetallicRoughness: { baseColorFactor: [parseInt(color.slice(1,3),16)/255, parseInt(color.slice(3,5),16)/255, parseInt(color.slice(5,7),16)/255, 1], metallicFactor: 0.1, roughnessFactor: 0.75 } }],
    accessors: [{ bufferView: 0, componentType: 5126, count: 3, type: 'VEC3', min: [-0.5,0,-0.5], max: [0.5,1,0.5] }, { bufferView: 1, componentType: 5123, count: 3, type: 'SCALAR' }],
    ...
    extras: { prompt: input.prompt, deterministicSeed: hash, spatialModelContract },
  }, null, 2));
}
...
function renderAudio(input: GenerateRequest & Record<string, unknown>, sampleRate: number, durationSeconds: number) {
  ...
  for (let index = 0; index < samples; index += 1) {
    ...
    view.setInt16(44+index*2, Math.max(-1,Math.min(1,Math.sin((2*Math.PI*frequency*index)/sampleRate)*envelope*0.35))*32767, true);
  }
...
function renderBundle(input: GenerateRequest & Record<string, unknown>) {
  ...
  return Buffer.from(JSON.stringify({ jobId: input.jobId, tenantId: input.tenantId ?? 'default', prompt: input.prompt, type: input.type, bundleVersion: 1, assets }, null, 2));
}
...
export async function renderAsset(input: GenerateRequest & Record<string, unknown>) {
  ...
  const spatialModelContract = definition.canonicalType === 'model3d' ? normalizeSpatialModelContract((input.metadata as Record<string, unknown> | undefined)?.spatialModelContract) : null;
  ...
  if (definition.canonicalType === 'model3d') {
    const manifest = buildManifest(input,{ rendererMode: definition.rendererMode, formats: definition.formats, width, height, previewPath:null, metadata:{ format, spatial:{ coordinateSystem:'right-handed-y-up', unit:'meter' }, spatialModelContract } });

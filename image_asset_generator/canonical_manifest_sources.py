"""Canonical manifest source builders for URAI V1, V2, and V4 XR assets.

Release meaning is defined only by canonical_version_catalog.json and
canonical_release_manifests.py. This module supplies deterministic source entries;
it does not choose version labels or write production receipts.
"""
...
def v1_manifest() -> list[dict[str, Any]]:
...
def v2_manifest() -> list[dict[str, Any]]:
...
def v4_xr_manifest() -> list[dict[str, Any]]:
    entries: list[dict[str, Any]] = []
    for folder, slug, purpose, claim_gate in V4_XR_GRAPHICS:
        ...
        entries.append(
            _entry(
                name=f"v4_{folder}_{slug.replace('-', '_')}",
                category=f"v4_{folder.replace('-', '_')}",
                path=f"{folder}/{slug}",
                purpose=purpose,
                size=size,
                aspect_ratio=ratio,

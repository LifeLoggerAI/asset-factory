"""Compatibility entry point for the canonical URAI V1–V5 production forge.

All release meaning comes from canonical_version_catalog.json and
canonical_release_manifests.py. This file preserves the historical command path
without preserving the historical conflicting catalog.
"""
...
def load_catalog() -> Dict[str, Any]:
    ...
    if not isinstance(versions, dict) or not versions:
        raise ValueError(
            "canonical_version_catalog.json must contain a non-empty versions object"
        )
    return payload
...
def resolve_version(name: str) -> tuple[Dict[str, Any], Path]:
    ...
    if name not in versions:
        raise ValueError(
            f"Unknown URAI version {name!r}; expected one of {', '.join(sorted(versions))}"
        )
...
def canonical_paths(
    ...
    config, _ = resolve_version(version)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the canonical URAI versioned asset forge"
    )
    parser.add_argument(
        "--version",
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="Print the canonical version catalog and exit",
    )
    ...
    built_manifest = build_selected_manifest(version)
    config, selected_manifest = resolve_version(version)
    if built_manifest.resolve() != selected_manifest.resolve():
        raise RuntimeError(
            f"Canonical builder returned {built_manifest}, expected {selected_manifest}"
        )
    ...
    if expected and len(entries) != expected:
        raise ValueError(
            f"{version} catalog expects {expected} assets but "
            f"{selected_manifest.name} has {len(entries)}"
        )
